// The idea here is to start adding postgresql support. We are also adding some debug output since pricelists and pricelist Items aren't playing nicely.
// I am creating this as a separate file as the original seems to work well for pulling pure json records down to disk.
// 
// Build with:
    ...
    class Client {
        ...
        bool updatePagination(const Endpoint& endpoint, const Json::Value& meta, const Json::Value& items, 
                    ...
                    if (meta.isMember("MetaCollectionResult")) {
                        ...
                        debug("MetaCollectionResult updated");
                    }
                    ...
                    switch (endpoint.pagination) {
                        ...
                        case PaginationType::ID: {
                            if (!meta.isMember("LastID")) return false;
                            int64_t new_id = meta["LastID"].asInt64();
                            ...
                            return items.size() > 0;  // Continue if items exist, ignore TotalCount
                        }
                        case PaginationType::SKIP: {
                ...
                void saveJson(const std::string& filename, const Json::Value& json, const std::string& endpoint_name) {
                    ...
                    try {
                        ...
                        std::string query = "INSERT INTO repsly.raw_data (endpoint_name, data) VALUES ($1, $2)";
                        txn.exec_params(query, endpoint_name, json_str);
                        txn.commit();
                    } catch (const pqxx::sql_error& e) {
                        std::cerr << "Database error: " << e.what() << " Query: " << e.query() << std::endl;
                        debug("Database error: " + std::string(e.what()) + " for query: " + e.query());
                ...
                Json::Value fetchEndpoint(const Endpoint& endpoint, PaginationState state = {}) {
                    ...
                    if (!page || (!page.isMember(endpoint.key) && !page.empty())) {
                        ...
                        if (endpoint.name == "pricelists") {
                            ...
                            if (pricelists.isNull() || pricelists.empty()) {
                                ...
                                debug("Pricelists fetched successfully");
                            }
                        ...
                        if (!no_pagination_ && endpoint.pagination != PaginationType::NONE) {
                            ...
                            while (has_more) {
                                ...
                                page = fetchPage(url);
                                if (!page || (!page.isMember(endpoint.key) && !page.empty())) {
                                    debug("Pagination request failed for " + endpoint.name + ", URL: " + url);
                                    skipped_ids.push_back(state.last_id);
                                    consecutive_failures++;
                                    if (consecutive_failures >= 3) {
                                        debug("Too many consecutive failures (" + std::to_string(consecutive_failures) + "), stopping pagination");
                                        failure_note = "(partial data, skipped IDs: " + std::to_string(skipped_ids.size()) + ")";
                                        break;
                                    }
                                    has_more = updatePagination(endpoint, state.meta, Json::Value(Json::arrayValue), state, progress);
                                    continue;
                                }
                                ...
                                has_more = updatePagination(endpoint, state.meta, items, state, progress);
                            ...
                            if (!skipped_ids.empty()) {
                                ...
                                for (const auto& id : skipped_ids) {
                                    skipped_list += id + " ";
                                }
                                debug("Skipped IDs: " + skipped_list);
                                if (failure_note.empty()) {
                                    failure_note = "(partial data, skipped " + std::to_string(skipped_ids.size()) + " IDs)";
                                }
                            }
                        } else {
                            reportProgress(endpoint.name, progress);
                    ...
                    if (!failure_note.empty()) {
                        ...
                        std::cout << "[" << progress.processed << " fetched] ";
                ...
                void fetchPricelistsItemsFromActive() {
                    debug("Fetching active pricelist IDs from pricelists.pricelist_active");
                    try {
                        pqxx::work txn(*db_conn_);
                        std::string query = "SELECT pricelist_id FROM pricelists.pricelist_active WHERE active = true";
                        pqxx::result result = txn.exec(query);
                        txn.commit();  // Commit the read transaction
        
                        for (const auto& row : result) {
                            int pricelist_id = row["pricelist_id"].as<int>();
                            std::string pricelistIdStr = std::to_string(pricelist_id);
                            debug("Fetching pricelistsItems for pricelist ID: " + pricelistIdStr);
                            ...
                            PaginationState items_state{
                                ...
                                .last_id = pricelistIdStr,
                                .skip = 0,
                                .meta = Json::Value()
                            };
                            Json::Value items = fetchEndpoint(items_endpoint, items_state);
                            if (!items.empty()) {
                                saveJson("repsly_pricelistsItems_" + pricelistIdStr + "_raw.json", items, "pricelistsItems");
                            } else {
                                debug("No items returned for pricelist ID " + pricelistIdStr);
                            }
                        }
                    } catch (const pqxx::sql_error& e) {
                        std::cerr << "Database error: " << e.what() << " Query: " << e.query() << std::endl;
                        debug("Database error in fetchPricelistsItemsFromActive: " + std::string(e.what()) + " for query: " + e.query());
                        throw std::runtime_error("Failed to fetch active pricelists from database");
                    }
                }
                ...
                explicit Client(const std::string& config_path, bool no_pagination = false)
                    ...
                    std::string db_config = "dbname=repsly user=sadf password=aasdfasdf host=localhost port=5432";
                    try {
                ...
                void fetchAll() {
                    ...
                    fetchPricelistsItemsFromActive();
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        Repsly::Client client("config.conf", no_pagination);
        client.fetchAll();
    } catch (const std::exception& e) {
